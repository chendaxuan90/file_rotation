...
#------------------------------------------------------
    if ($RETURN_VALUE -eq 0){
        ...
        if( $PSVersionTable.PSVersion.Major -ge 3 ){
            # Write-Host "The Path is $PSScriptRoot"
            $ScriptDir = $PSScriptRoot
            Write-Host "Start rotation......"
        }
        # for PS v2
        else{
            # Write-Host "The path is $MyInvocation.MyCommand.Path"
...
#------------------------------------------------------
    if ($RETURN_VALUE -eq 0){
        ...
        # dest folder path
        $destinationFolderPath = "$SCRIPT_DIR\0.0 old"
...
#------------------------------------------------------
    if ($RETURN_VALUE -eq 0){
        ...
        foreach ($file in $files) {
            # Check if the file is not a .ps1 file
            if ($file.Extension -notmatch "\.ps1$") {
                # Check last write time
                if ($file.LastWriteTime -lt $oneMonthAgo) {
                    # Move file from list
                    $filesToMove += $file
                }
            }
